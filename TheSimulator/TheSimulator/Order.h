...
constexpr const char* ORDERID_INVALID = "";
...
public:
	inline const OrderID& id() const { return m_id; }
	inline Timestamp timestamp() const { return m_timestamp;  }
	inline Volume volume() const { return m_volume; }
	inline const std::string& sourceAgent() const { return m_sourceAgent; }
...
protected:
	BasicOrder(const OrderID& id, const std::string& sourceAgent, Timestamp timestamp, Volume orderVolume);
...
protected:
	Order(const OrderID& id, const std::string& sourceAgent, OrderDirection orderDirection, Timestamp timestamp, Volume orderVolume);
private:
...
public:
	...
	void printCSV() const override;
protected:
	MarketOrder(const OrderID& id, const std::string& sourceAgent, OrderDirection direction, Timestamp timestamp, Volume volume);
...
public:
	...
	LimitOrder& operator=(const LimitOrder&) { 
		// TODO: WARNING: DANGER: BAD CODE, RETHINK THIS WHOLE HEADER FILE
		return *this;
	}
protected:
	LimitOrder(const OrderID& id, const std::string& sourceAgent, OrderDirection direction, Timestamp timestamp, Volume volume, const Money& price);
